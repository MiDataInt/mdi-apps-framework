...
#----------------------------------------------------------------------
# regardless of how Shiny::runApp was called, it runs in .../shiny/shared
#----------------------------------------------------------------------
# global.R is called:
...
if(isParentProcess){
    ...
    for (var in c('SHARED_DIR', 'PORTAL_DIR',
                  'SERVER_MODE','DEBUG',
                  'PORTAL_VERSION','BIOCONDUCTOR_RELEASE',
                  'MAX_MB_RAM_BEFORE_START','MAX_MB_RAM_AFTER_END')){
        if(is.null(serverEnv[[var]])) stop(paste("missing variable:", var))
    }   
    
    # check for required directories
    for (dir in c('PORTAL_DIR','SHARED_DIR','LIBRARY_DIR')){
...
# load and attach initial Shiny load dependencies of the MAGC Portal framework
...
for(familyName in list.dirs(path=file.path(serverEnv$SHINY_DIR, 'apps'), full.names=FALSE, recursive=FALSE)){ 
    for(appName in list.dirs(path=file.path(serverEnv$SHINY_DIR, 'apps', familyName), full.names=FALSE, recursive=FALSE)){
        configFile <- file.path(serverEnv$SHINY_DIR, 'apps', familyName, appName, 'config.yml')
        if(file.exists(configFile)){ tryCatch({ # tryCatch prevent framework failure on bad config
