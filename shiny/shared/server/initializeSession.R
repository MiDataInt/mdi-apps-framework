...
globusUserData <- list()    # user info+tokens associated with Globus login (session-specific, not always required)
headerStatusData <- reactiveValues( # for UI display
    user = if(serverEnv$IS_GLOBUS) "" else paste(Sys.getenv(c('USERNAME','USER')), collapse=""),
...
# activate our custom page reset action; reload the page as is, to update all code
observeEvent(input$resetPortalPage, {
    updateQueryString("?resetPortalPage=1", mode="push") # clear the url
    refresh()
})
