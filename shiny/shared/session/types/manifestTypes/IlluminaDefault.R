...
#----------------------------------------------------------------------
# standard Illumina demux file, typically multiple rows per sample
#----------------------------------------------------------------------
...
parseIlluminaManifest <- function(manifest){
    ...
    # first calculate the sum of the columns
    unique <- aggregate( 
    ...
    for(col in IlluminaSampleValueMeanColumns){
        unique[[col]] <- sapply(1:nrow(unique), function(i){
