...
getBookmarkJson <- function(){
    ...
    for(stepName in names(app$info$appSteps)){ # convert reactives to hard values
        ...
        if(!is.null(stepData$input))    bookmark$input[[stepName]]    <- reactiveValuesToList(stepData$input) # always reactiveValues
        if(!is.null(stepData$settings)) bookmark$settings[[stepName]] <- stepData$settings() # always a reactive or reactiveVal
...
loadBookmarkFromString <- function(bookmark){
    path <- tempfile(pattern="bookmark", fileext = ".magc")
